...
1. join the tracked `search_terms` into one source query
2. fetch the KaBuM search page for that query
3. parse `script#__NEXT_DATA__`
4. read `catalogServer.data` to collect listing items
5. keep only items whose title contains all tracked search terms, ignoring case
6. persist only the 4 lowest matching prices for that source-specific run
...
- listing titles are stable enough for case-insensitive containment matching against all tracked search terms
...
- titles changing enough that the all-terms containment rule no longer matches the intended product
